...
train = pd.read_csv('G:/hoip_data/HOIP_cif/input/Train14Features1012_re.csv')
...
ntest = test.shape[0]
y_train = train.Relative_energy1.values
all_data = pd.concat((train, test)).reset_index(drop=True)
all_data.drop(['Relative_energy1'], axis=1, inplace=True)
 
train = all_data[:ntrain]
test = all_data[ntrain:]
 
train.to_csv('train.csv', header=None, index=None, sep=' ', mode='a')
non_features = ['Relative_energy1']
...
def plot_actual_pred(train_pred, train_actual,
    ...
    ax = plt.plot([xmin, xmax], [ymin, ymax],
                  ...
                  label='__nolegend__')
    ax = plt.xlabel('Actual relative energy $\epsilon^{rel}_{HOIP}$', fontsize=fontsize)
    ax = plt.ylabel('Predicted relative energy $\epsilon^{rel}_{HOIP}$', fontsize=fontsize)
...
sub.to_csv('predict_re_1013.csv',index=False)
 
target_E = 'Relative_energy1'
print('RMSLE for relative energy = %.3f eV (training) and %.3f eV (test)'
